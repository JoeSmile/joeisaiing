...
> **说明**：下文描述的是 RAG 聊天场景的**安全设计思路与行业通用做法**，并非某套系统的生产配置、规则明细或现网能力承诺。具体阈值、开关策略、Prompt 原文与检测规则属于运维细节，**不在公开文中披露**。
...
以下是在 RAG 项目里**常见、值得 benchmark** 的开源组件；是否启用、启用哪几层，应**按环境与数据单独决策**，公开文不讨论具体开关策略。
...
| 包 | 适用层 | 说明 |
| :--- | :--- | :--- |
| [LLM Guard](https://github.com/protectai/llm-guard) | 用户输入 / 可选输出 | Prompt 注入检测、不可见字符等；需 benchmark 延迟与误杀 |
| [llm-injection-guard](https://github.com/maheshmakvana/llm-injection-guard) | FastAPI 中间件 | 轻量；适合与自研规则**对照实验** |
| [EntropyShield](https://pypi.org/project/entropyshield/) | 检索块清洗链 | DeSyntax；需用**业务正例**测误伤 |
...
| 垂直领域 RAG | 上栏 + 限流 | + ML Guard + DeSyntax（经评测） |
...
试用任何 Guard，建议准备两套数据集（**具体内容与样本库不对外公开**）：

| 集合 | 内容 |
| :--- | :--- |
| **正例** | 正常业务问答；含容易误杀的领域用语 |
| **反例** | 多语言 override、混淆字符、资料内嵌指令等合成样本 |

指标：**拦截率、误杀率、p50/p99 延迟、回归测试是否仍绿**。

推荐试用顺序：

1. 自研输入 Guard + 检索清洗 + 分层 Prompt（底座）  
2. BFF 轻量 Guard（对照 UX）  
3. 轻量 API 中间件（对照实验）  
4. ML Guard（记录延迟与误杀）  
5. DeSyntax on chunk 链（测误伤）  
6. Garak 探针（CI 非阻塞）  
...
- 输入上限、限流阈值、Guard 开关：**仅通过配置中心 / 环境变量管理，不在博客或仓库文档写死默认值**。  
- 拒绝响应：**统一错误码与文案**，不向客户端返回「命中哪条规则」。  
- 规则与 Prompt：**版本化管理**，变更走 Code Review + 红队/回归。  
- 分层启用：开发 / 预发 / 生产可不同；**以预发评测结果为准**，而非照搬某篇公开文章。

## 八、小结

LLM 聊天安全没有「装一个 npm 包就完事」的捷径。工程上更稳的路径是：

- **硬防线**：长度、规则、RAG 双点清洗、限流配额  
- **软防线**：策略/文档分层 + 消息角色分离  
- **可选增强**：ML Guard、DeSyntax、云 API、红队探针——经 Adapter 试用，用**自家数据**决定生产组合  

若你也在做 RAG 产品，建议从 **间接注入 + 快失败** 两条线先补齐，再考虑 ML 层——往往性价比最高。公开分享架构思路有助于社区交流；**真正防住攻击**靠的是实现质量、运维策略和持续评测，而不是在文章里藏规则。
